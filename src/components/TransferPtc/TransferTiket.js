...
export default function TransferTiket() {
    const [value, setValue] = useState(null);
    return (
        <View style={styles.Container}>
            <View style={styles.Head}>
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <Image style={styles.Image} source={require('../../assets/images/PTC/IconTiket.png')} />
                    <Text style={{ color: color.fontBody2, fontSize: sizeFont(3.3) }}>Jumlah Ticket</Text>
                </View>
                <Text style={{ fontFamily: Poppins.Medium, fontSize: sizeFont(3.5) }}>137 Ticket</Text>
            </View>
                    ...
                    [10, 25, 50].map((item, index) => {
                        return (
                            <TouchableOpacity onPress={() => setValue(item.toString())} activeOpacity={0.6} key={index} style={styles.Instan}>
                                <Text style={{ fontSize: sizeFont(3), color: color.fontBody2, fontFamily: Poppins.Medium }}>{item} Ticket</Text>
                            </TouchableOpacity>
                        );
                    })
                }
